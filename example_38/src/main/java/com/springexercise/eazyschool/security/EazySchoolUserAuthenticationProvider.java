...
public class EazySchoolUserAuthenticationProvider  implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if( null!= person && person.getPerson_id()>0 && person.getPwd().equals(pwd)){
            return new UsernamePasswordAuthenticationToken(person.getName(), pwd, getGrantedAuthority(person.getRoles()));
        } else {
